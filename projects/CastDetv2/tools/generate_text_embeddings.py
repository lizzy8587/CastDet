...
DOTA_CATEGORIES=('plane', 'baseball diamond', 'bridge', 'groundtrackfield',
             'small vehicle', 'large vehicle', 'ship', 'tenniscourt',
             'basketballcourt', 'storagetank', 'soccerball field',
             'roundabout', 'harbor', 'swimming pool', 'helicopter')

DOTA_BGS=('plane', 'baseball diamond', 'bridge', 'groundtrackfield',
             'small vehicle', 'large vehicle', 'ship', 'tenniscourt',
             'basketballcourt', 'storagetank', 'soccerball field',
             'roundabout', 'harbor', 'swimming pool', 'helicopter',
             'chimney', 'dam', 'Expressway-Service-area',
           'Expressway-toll-station', 'golffield', 'overpass',
           'trainstation', 'airport', 'windmill', 'low scattered building', 'industrial area', 'desert',
           ...
           'parking lot', 'sewage plant-type-two', 'thermal power plant',
...
def generate_clip_text_embeddings(save_path, model_path, text_queries, add_bg=False):
    ...
    # text_queries = [f"a {i}" for i in text_queries]   # template1
    # text_queries = [f"{i}" for i in text_queries]   # template2
    # text_queries = [f"a satellite photo of {i}" for i in text_queries]   # template3
    text_queries = [f"a photo of {i}" for i in text_queries]    # default
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_queries', nargs='+', default=DOTA_BGS, help='List of text queries')
