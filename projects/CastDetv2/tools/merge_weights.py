...
def merge_weights(clip_path, base_path, save_path, base_model='soft-teacher', target_model='castdet'):
    clip = torch.load(clip_path)#.state_dict()
    base = torch.load(base_path)
    save_dict = {}
    if target_model == 'castdet':
    ...
    elif target_model == 'vild':
        save_dict = base['state_dict']
...
def main():
    ...
    parser.add_argument("--target_model", type=str, default="castdet", choices=["castdet", "vild"], help="Target model type: 'castdet' or 'vild'")
