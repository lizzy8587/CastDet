...
class CastDet(SemiBaseDetector):
    def __init__(self,
        ...
        self.semi_loss = semi_train_cfg.get('semi_loss', 'cross_entropy')
        ...
        self.fair_seim_loss = semi_train_cfg.get('fair_seim_loss', False)
        self.test_with_gt = semi_train_cfg.get('test_with_gt', False)
        self.test_with_clip = semi_test_cfg.get('test_with_clip', False)
        self.test_with_gt_clip = semi_test_cfg.get('test_with_gt_clip', False)
        ...
        self.semi_branch = semi_train_cfg.get('semi_branch', 2)
    ...
    def predict(self, batch_inputs: Tensor, batch_data_samples: SampleList) -> SampleList:
        if self.test_with_gt is True:
            results = self.predict_with_gt(batch_inputs, batch_data_samples)
        elif self.test_with_clip is True:
            results = self.predict_with_clip(batch_inputs, batch_data_samples)
        else:
            results = super().predict(batch_inputs, batch_data_samples)
        return results
    
    def predict_with_clip(self,
                batch_inputs: Tensor,
        ...
        if self.test_with_gt_clip:
            rpn_data_samples = copy.deepcopy(batch_data_samples)
            for data_sample in rpn_data_samples:
                data_sample.gt_instances.labels = torch.zeros_like(data_sample.gt_instances.labels)
                data_sample.gt_instances.scores = torch.ones_like(data_sample.gt_instances.labels, dtype=float)
            rpn_results_list = [data_sample.gt_instances for data_sample in rpn_data_samples]
        else:
        ...
        for idx, (inputs, rpn_result, data_samples) in enumerate(zip(batch_inputs, rpn_results_list, batch_data_samples)):
            # data_samples.pred_instances = get_empty_instance(self.device)
            if not self.test_with_gt_clip:
                ...
                rpn_result = self.filter_bboxes(rpn_result)
            ...
            rpn_result.labels = labels
            rpn_result.scores = scores
            ids = labels < num_classes
            rpn_results_list[idx] = rpn_result[ids] if ids.sum() > 0 else get_empty_instance(self.device)

        batch_data_samples = self.teacher.add_pred_to_datasample(
    ...
    def initialize_pseudo_queue(self):
        for img_path in tqdm(self.pseudo_queue.init_imgs):
            ...
            # save_annotated(img_path, rpn_result.bboxes[rpn_result.scores > 0.9], f"work_dirs/vis_data/box_reg_test/{img_path.split('/')[-1]}")
            rpn_result = self.filter_bboxes(rpn_result, save_proposals)
            if len(rpn_result) == 0:
                continue
    ...
    def filter_bboxes(self, rpn_result, save_proposals=None):
        ...
        if save_proposals is not None and self.semi_jitter_num > 0:
            ...
            # bboxes[:,:,1] = bboxes[:,:,1] / h
            # bboxes[:,:,3] = bboxes[:,:,3] / h
            # var = torch.var(bboxes, dim=0).sum(dim=-1)
    ...
    def loss_by_semi_instances(self) -> dict:
        ...
        # reweight for cls loss
        if self.fair_seim_loss:
            losses['semi_loss_cls'] *= len(labels) / (num_imgs * sampling_result.avg_factor)
...
def get_empty_instance(device):
    empty_instance = InstanceData()
    empty_instance.bboxes = torch.zeros(0, 4, device=device)
    empty_instance.scores = torch.zeros((0, ), device=device)
    empty_instance.labels = torch.zeros((0, ), device=device, dtype=torch.long)
    return empty_instance

def save_annotated(img_path, bboxes, save_path=None):
    img = cv2.imread(img_path)
    for bbox in bboxes:
        color = np.random.randint(0, high=255, size=(3,)).tolist()
        img = cv2.rectangle(img, (int(bbox[0]), int(bbox[1])), (int(
            bbox[2]), int(bbox[3])), color, thickness=2)
