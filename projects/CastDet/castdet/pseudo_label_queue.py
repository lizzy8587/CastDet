...
import imagesize
import cv2
from mmdet.datasets.transforms import FixShapeResize as mmFixResize
...
def read_voc_xml(file_path, classes2id):
    ...
    for obj in root.findall('object'):
        ...
        labels.append(classes2id[label])
    ...
    return filename, torch.tensor(labels), torch.tensor(bboxes)
...
class PseudoQueue:
    def __init__(
        ...
        **kwargs
        ...
        self.without_queue = kwargs.get('without_queue', False)
        self.cur_iter = 0
        self.init_imgs = []
    ...
    def initialize(self, **kwargs):
        if (kwargs.get('ann_file', False) and kwargs.get('img_dir', False) and
            kwargs.get('ann_dir', False) and kwargs.get('classes', False)):
            ann_file = kwargs['ann_file']
            img_dir = kwargs['img_dir']
            ann_dir = kwargs['ann_dir']
            classes = kwargs['classes']
            classes2id = dict(zip(classes, range(len(classes))))
            with open(ann_file, 'r') as fp:
                data = fp.readlines()
                file_paths = [os.path.join(ann_dir, i.strip('\n')+'.xml') for i in data]
            for file_path in file_paths:
                filename, labels, bboxes = read_voc_xml(file_path, classes2id)
                img_path = os.path.join(img_dir, filename)
                self.update_pseudo_queue(img_path, bboxes, labels)
    ...
    def update_pseudo_queue(self, img_path, bboxes, labels, scores=None):
        ...
        if to_update:
            # gt_detSample = self.bbox2detSample(img_path, bboxes, labels, scores)
            # self.name2detSample[img_name] = gt_detSample
    ...
    def get_all_and_clear(self):
        imgs, detsample_list = [], []
        samples, results = [], []
        for sample in self.name2detSample.values():
        ...
        for k in self.unseen_imgs.keys():
            self.unseen_imgs[k] = []
        self.name2detSample = {}
    ...
    def get_image_detsample(self):
        ...
        if self.without_queue:
            return self.get_all_and_clear()
        imgs, detsample_list = [], []
        if not self.start_training():
            return imgs, detsample_list

        # img_list = [random.choice(self.unseen_imgs[random.choice(self.unseen_ids)]) for i in range(self.batch_num)]
